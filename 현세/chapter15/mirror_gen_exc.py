...
def looking_glass():
    ...
    msg = ''  # <1>
    '''
    @contextmanager를 사용할 때는 어쩔 수 없이 yield 문 주변을 try/finally나 with 블록으로 둘러싸야 한다.
    컨텍스트 관리자의 사용자가 자신의 with 블록 안에서 어떤 일을 할 지 모르기 때문이다.
    '''
    try:
        yield 'JABBERWOCKY'
        ...
        sys.stdout.write = original_write  # <3> 멍키 패칭한 sys.stdout.write()를 원래대로 복원한다.
        if msg:
            print(msg)  # <4>
